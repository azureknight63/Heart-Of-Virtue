...
- src/api/routes/quest_rewards.py  (16% → ~75%)
- src/api/routes/quest_chains.py  (18% → ~80%)
...
def _make_game_service():
    ...
    gs.get_quest_rewards.return_value = {"success": True, "rewards": []}
    gs.complete_quest.return_value = {"success": True, "rewards": {}}
    gs.award_gold.return_value = {"success": True, "gold": 100}
    gs.award_experience.return_value = {"success": True, "exp": 200}
    gs.award_item.return_value = {"success": True, "item": "sword"}
    gs.award_reputation.return_value = {"success": True}
    gs.get_player_progression.return_value = {"success": True, "progression": {}}
    gs.get_all_chains_progress.return_value = {"all_chains": []}
    gs.get_chain_progress.return_value = {"progress": {}}
    gs.advance_chain_stage.return_value = {"advancement": {}}
    gs.complete_chain.return_value = {"completion": {}}
...
# ===========================================================================
# quest_rewards routes
# ===========================================================================


class TestQuestRewardsRoutes:
    """Test quest rewards API endpoints."""

    @pytest.fixture
    def client(self):
        from src.api.routes.quest_rewards import quest_rewards_bp

        app = _make_minimal_app([(quest_rewards_bp, None)])
        with app.test_client() as c:
            yield c, app

    def test_get_quest_rewards_no_auth(self, client):
        c, _ = client
        rv = c.get("/api/quests/q1/rewards")
        assert rv.status_code == 401

    def test_get_quest_rewards_success(self, client):
        c, _ = client
        rv = c.get("/api/quests/q1/rewards", headers=AUTH_HEADER)
        assert rv.status_code == 200

    def test_get_quest_rewards_not_found(self, client):
        c, app = client
        app._test_gs.get_quest_rewards.return_value = {"success": False}
        rv = c.get("/api/quests/q1/rewards", headers=AUTH_HEADER)
        assert rv.status_code == 404
    ...
    def test_complete_quest_failure(self, client):
        c, app = client
    ...
    def test_award_gold_debug_only(self, client):
        c, _ = client
        rv = c.post("/api/quests/award-gold", json={"amount": 100}, headers=AUTH_HEADER)
        # Debug mode is True in our app fixture
        assert rv.status_code == 200

    def test_award_gold_missing_amount(self, client):
        c, _ = client
        rv = c.post("/api/quests/award-gold", json={}, headers=AUTH_HEADER)
        assert rv.status_code == 400

    def test_award_gold_invalid_amount(self, client):
        c, _ = client
        rv = c.post("/api/quests/award-gold", json={"amount": -50}, headers=AUTH_HEADER)
        assert rv.status_code == 400

    def test_award_experience_success(self, client):
        c, _ = client
        rv = c.post(
            "/api/quests/award-experience", json={"amount": 200}, headers=AUTH_HEADER
        )
        assert rv.status_code == 200

    def test_award_experience_missing_amount(self, client):
        c, _ = client
        rv = c.post("/api/quests/award-experience", json={}, headers=AUTH_HEADER)
        assert rv.status_code == 400

    def test_award_item_success(self, client):
        c, _ = client
        rv = c.post(
    ...
    def test_award_item_missing_fields(self, client):
        c, _ = client
        rv = c.post(
            "/api/quests/award-item", json={"item_id": "s1"}, headers=AUTH_HEADER
        )
        assert rv.status_code == 400

    def test_award_item_invalid_quantity(self, client):
        c, _ = client
        rv = c.post(
            "/api/quests/award-item",
            json={"item_id": "s1", "item_name": "Sword", "quantity": 0},
            headers=AUTH_HEADER,
        )
        assert rv.status_code == 400

    def test_award_item_failure(self, client):
        c, app = client
        app._test_gs.award_item.return_value = {"success": False, "error": "inv full"}
        rv = c.post(
            "/api/quests/award-item",
            json={"item_id": "s1", "item_name": "Sword"},
            headers=AUTH_HEADER,
        )
        assert rv.status_code == 400
    ...
    def test_award_reputation_success(self, client):
        ...
        rv = c.post(
            ...
            json={"npc_id": "mynx", "npc_name": "Mynx", "amount": 10},
            headers=AUTH_HEADER,
        )
        assert rv.status_code == 200
    ...
    def test_award_reputation_non_int_amount(self, client):
        ...
        rv = c.post(
            "/api/quests/award-reputation",
            json={"npc_id": "mynx", "npc_name": "Mynx", "amount": "lots"},
            headers=AUTH_HEADER,
        )
        assert rv.status_code == 400
    ...
    def test_get_progression(self, client):
        c, _ = client
        rv = c.get("/api/quests/progression", headers=AUTH_HEADER)
        assert rv.status_code == 200
...
class TestQuestChainsRoutes:
    """Test quest chain API endpoints."""

    @pytest.fixture
    def client(self):
        from src.api.routes.quest_chains import quest_chains_bp

        app = _make_minimal_app([(quest_chains_bp, None)])
        with app.test_client() as c:
            yield c, app

    def test_get_all_chains_no_auth(self, client):
        c, _ = client
        rv = c.get("/api/quest-chains/progress")
        assert rv.status_code == 401

    def test_get_all_chains_success(self, client):
        c, _ = client
        rv = c.get("/api/quest-chains/progress", headers=AUTH_HEADER)
        assert rv.status_code == 200
        assert rv.get_json()["success"] is True

    def test_get_chain_progress_success(self, client):
        c, _ = client
        rv = c.get("/api/quest-chains/chain_01/progress", headers=AUTH_HEADER)
        assert rv.status_code == 200

    def test_advance_chain_missing_current_stage(self, client):
        c, _ = client
        rv = c.post(
            "/api/quest-chains/chain_01/advance",
            json={"next_stage": 2},
            headers=AUTH_HEADER,
        )
        assert rv.status_code == 400
    ...
    def test_advance_chain_negative_next_stage(self, client):
        c, _ = client
        rv = c.post(
            "/api/quest-chains/chain_01/advance",
    ...
    def test_advance_chain_success(self, client):
        c, _ = client
        rv = c.post(
            "/api/quest-chains/chain_01/advance",
            json={"current_stage": 0, "next_stage": 1},
    ...
    def test_complete_chain_success(self, client):
        c, _ = client
        rv = c.post("/api/quest-chains/chain_01/complete", headers=AUTH_HEADER)
        assert rv.status_code == 200
        assert rv.get_json()["success"] is True

    def test_check_prerequisites_success(self, client):
        c, _ = client
        rv = c.post(
            "/api/quest-chains/chain_01/prerequisites",
            json={"prerequisites": ["chain_00"]},
            headers=AUTH_HEADER,
        )
        assert rv.status_code == 200

    def test_check_prerequisites_empty_body(self, client):
        c, _ = client
        rv = c.post(
            "/api/quest-chains/chain_01/prerequisites",
            json={},
    ...
    def test_check_prerequisites_invalid_type(self, client):
        c, _ = client
        rv = c.post(
            "/api/quest-chains/chain_01/prerequisites",
            json={"prerequisites": "not_a_list"},
            headers=AUTH_HEADER,
        )
        assert rv.status_code == 400
...
class TestNpcRoutes:
    ...
    def test_get_active_quests(self, client):
        c, _ = client
        rv = c.get("/api/npc/quests/active", headers=AUTH_HEADER)
        assert rv.status_code == 200

    def test_accept_quest_empty_id(self, client):
        c, _ = client
        # Route requires non-empty quest_id — empty string hits 404 on Flask routing
        rv = c.post("/api/npc/quests//accept", headers=AUTH_HEADER)
        assert rv.status_code == 404

    def test_accept_quest_success(self, client):
        c, _ = client
        rv = c.post("/api/npc/quests/q01/accept", headers=AUTH_HEADER)
        assert rv.status_code == 200

    def test_update_quest_progress_missing_objective(self, client):
        c, _ = client
        rv = c.post("/api/npc/quests/q01/progress", json={}, headers=AUTH_HEADER)
        assert rv.status_code == 400

    def test_update_quest_progress_success(self, client):
        c, _ = client
        rv = c.post(
            "/api/npc/quests/q01/progress",
    ...
    def test_get_quest_status_success(self, client):
        c, _ = client
        rv = c.get("/api/npc/quests/q01/status", headers=AUTH_HEADER)
        assert rv.status_code == 200
