...
class TestEventIntegration:
    ...
    def test_tile_entry_triggers_events(self, app, client, session_id):
        ...
        # Setup: Add an event to the tile at (0,1)
        tile = app.game_service.universe.get_tile(0, 1)
        if tile:
            event = MockEvent("Entering northern room", repeat=False)
        ...
        assert data["events_triggered"][0]["description"] == "Entering northern room"

    def test_multiple_events_on_tile(self, app, client, session_id):
        """Test that a tile with multiple events triggers all of them."""
        # Setup: Add multiple events to tile at (1,0)
        tile = app.game_service.universe.get_tile(1, 0)
        if tile:
    ...
    def test_movement_result_includes_event_consequences(self, app, client, session_id):
        ...
        # Setup: Add event to destination
        tile = app.game_service.universe.get_tile(0, 1)
        if tile:
            event = MockEvent("Consequence event")
            tile.events_here.append(event)

        # Action: Move to that tile
        response = client.post(
            "/world/move",
            json={"direction": "north"},
            headers={"Authorization": f"Bearer {session_id}"},
    ...
    def test_tile_without_events(self, app, client, session_id):
        ...
        # Setup: Ensure tile has no events
        tile = app.game_service.universe.get_tile(0, 1)
        if tile:
            tile.events_here = []
    ...
    def test_event_processing_on_movement(self, app, client, session_id):
        ...
        # Setup: Add event that tracks processing
        tile = app.game_service.universe.get_tile(0, 1)
        if tile:
...
class TestEventEdgeCases:
    ...
    def test_malformed_event_object(self, app, client, session_id):
        ...
        # Setup: Add event with missing attributes
        tile = app.game_service.universe.get_tile(0, 1)
        if tile:
        ...
        # Action: Move to tile with bad event
        response = client.post(
            "/world/move",
            json={"direction": "north"},
            headers={"Authorization": f"Bearer {session_id}"},
    ...
    def test_event_without_process_method(self, app, client, session_id):
        ...
        # Setup: Add event without process method
        tile = app.game_service.universe.get_tile(0, 1)
        if tile:
    ...
    def test_event_process_raises_exception(self, app, client, session_id):
        ...
        # Setup: Add event that raises exception
        tile = app.game_service.universe.get_tile(0, 1)
        if tile:
