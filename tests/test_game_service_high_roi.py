...
class TestEquipItem:
    """Test equip_item method - slot validation, stat modifications."""

    def test_equip_item_valid_slot(self, game_service, mock_player):
        """Test equipping item to valid slot."""
        mock_item = MagicMock()
        ...
        mock_player.inventory.get_item_by_index = MagicMock(return_value=mock_item)
    ...
    def test_equip_item_invalid_index(self, game_service, mock_player):
        """Test equipping item with invalid inventory index."""
        mock_player.inventory.get_item_by_index = MagicMock(return_value=None)
        mock_player.inventory.weight = 0
        mock_player.inventory.max_weight = 100

        result = game_service.equip_item(mock_player, 999)

        # Should handle error gracefully - returns dict with error
        assert result is not None
        assert isinstance(result, dict)

    def test_equip_item_wrong_slot_type(self, game_service, mock_player):
        """Test equipping item to wrong slot type."""
        mock_item = MagicMock()
        mock_item.slot = "weapon"
...
class TestUnequipItem:
    """Test unequip_item method - slot validation, inventory update."""

    def test_unequip_item_valid_slot(self, game_service, mock_player):
        """Test unequipping item from valid slot."""
        mock_item = MagicMock()
        mock_item.name = "Iron Sword"
