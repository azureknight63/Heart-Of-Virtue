...
- scenario_config.py (ScenarioConfig, DifficultyProgressionManager, ScenarioValidator)
...
class TestDifficultyProgressionManager:
    """Test DifficultyProgressionManager class."""

    def test_difficulty_progression_manager_init(self):
        """Test DifficultyProgressionManager initialization."""
        from src.scenario_config import ScenarioConfig, DifficultyProgressionManager

        player = Mock(spec=[])
        scenario_config = ScenarioConfig(player)
        manager = DifficultyProgressionManager(scenario_config)

        assert manager.scenario_config is scenario_config

    def test_get_difficulty_multiplier(self):
        """Test get_difficulty_multiplier."""
        from src.scenario_config import ScenarioConfig, DifficultyProgressionManager
        ...
        scenario_config = ScenarioConfig(player)
        scenario_config.combat_count = 2
        manager = DifficultyProgressionManager(scenario_config)

        multiplier = manager.get_difficulty_multiplier()
        # current = 3 + (2 * 0.5) = 4, multiplier = 4 / 3 = 1.333
        assert abs(multiplier - 4 / 3) < 0.01

    def test_get_difficulty_multiplier_zero_starting(self):
        """Test get_difficulty_multiplier with zero starting difficulty."""
        from src.scenario_config import ScenarioConfig, DifficultyProgressionManager

        player = Mock()
        player.game_config = Mock()
        player.game_config.starting_difficulty = 0
        player.game_config.difficulty_scaling = 0.5
    ...
    def test_apply_difficulty_to_npc_stats_damage(self):
        """Test apply_difficulty_to_npc_stats with damage."""
        from src.scenario_config import ScenarioConfig, DifficultyProgressionManager

        player = Mock()
        player.game_config = Mock()
        player.game_config.starting_difficulty = 2
        player.game_config.difficulty_scaling = 1.0

        scenario_config = ScenarioConfig(player)
        scenario_config.combat_count = 1  # difficulty = 2 + 1 = 3, multiplier = 1.5
        manager = DifficultyProgressionManager(scenario_config)

        npc = Mock()
        scaled = manager.apply_difficulty_to_npc_stats(npc, "damage", 10.0)
        assert scaled == 15.0

    def test_apply_difficulty_to_npc_stats_speed(self):
        """Test apply_difficulty_to_npc_stats with speed (smaller boost)."""
        from src.scenario_config import ScenarioConfig, DifficultyProgressionManager
        ...
        scenario_config = ScenarioConfig(player)
        scenario_config.combat_count = 1  # multiplier = 1.5
        manager = DifficultyProgressionManager(scenario_config)

        npc = Mock()
        scaled = manager.apply_difficulty_to_npc_stats(npc, "speed", 10.0)
        # speed gets 0.5 * (1.5 - 1.0) boost = 0.5 * 0.5 = 0.25 total multiplier
        # so 10 * (1 + 0.25) = 12.5
        assert abs(scaled - 12.5) < 0.01

    def test_get_enemy_count_by_difficulty(self):
        """Test get_enemy_count_by_difficulty."""
        from src.scenario_config import ScenarioConfig, DifficultyProgressionManager
        ...
        # 1.0x difficulty -> 2 enemies
        count = manager.get_enemy_count_by_difficulty(difficulty=1.0)
        assert count == 2

        # 1.5x difficulty -> 3 enemies
        count = manager.get_enemy_count_by_difficulty(difficulty=1.5)
        assert count == 3

        # 2.0x difficulty -> 4 enemies
        count = manager.get_enemy_count_by_difficulty(difficulty=2.0)
        assert count == 4

    def test_get_loot_multiplier(self):
        """Test get_loot_multiplier."""
        from src.scenario_config import ScenarioConfig, DifficultyProgressionManager
        ...
        multiplier = manager.get_loot_multiplier(difficulty=1.0)
        assert multiplier == 1.0

        # 2.0x difficulty
        multiplier = manager.get_loot_multiplier(difficulty=2.0)
        assert multiplier == 2.0

    def test_get_experience_multiplier(self):
        """Test get_experience_multiplier."""
        from src.scenario_config import ScenarioConfig, DifficultyProgressionManager
        ...
        multiplier = manager.get_experience_multiplier(difficulty=1.5)
        assert multiplier == 1.5

    def test_increment_difficulty_after_combat(self):
        """Test increment_difficulty_after_combat."""
        from src.scenario_config import ScenarioConfig, DifficultyProgressionManager
        ...
        new_difficulty = manager.increment_difficulty_after_combat()
        assert new_difficulty == 3.5  # 3 + (1 * 0.5)


class TestScenarioValidator:
    """Test ScenarioValidator class."""

    def test_scenario_validator_init(self):
        """Test ScenarioValidator initialization."""
        from src.scenario_config import ScenarioConfig, ScenarioValidator

        player = Mock(spec=[])
        scenario_config = ScenarioConfig(player)
        validator = ScenarioValidator(scenario_config)

        assert validator.scenario_config is scenario_config

    def test_is_valid_scenario_transition(self):
        """Test is_valid_scenario_transition."""
        from src.scenario_config import ScenarioConfig, ScenarioValidator
    ...
    def test_is_valid_scenario_transition_invalid_source(self):
        """Test is_valid_scenario_transition with invalid source."""
        from src.scenario_config import ScenarioConfig, ScenarioValidator
    ...
    def test_is_valid_difficulty_level(self):
        """Test is_valid_difficulty_level."""
        from src.scenario_config import ScenarioConfig, ScenarioValidator
    ...
    def test_is_valid_difficulty_level_negative(self):
        """Test is_valid_difficulty_level with negative value."""
        from src.scenario_config import ScenarioConfig, ScenarioValidator
    ...
    def test_is_valid_difficulty_level_too_high(self):
        """Test is_valid_difficulty_level with too-high value."""
        from src.scenario_config import ScenarioConfig, ScenarioValidator
        ...
        assert is_valid is False
        assert "unreasonably high" in reason.lower()

    def test_is_valid_round_count(self):
        """Test is_valid_round_count."""
        from src.scenario_config import ScenarioConfig, ScenarioValidator
    ...
    def test_is_valid_round_count_negative(self):
        """Test is_valid_round_count with negative value."""
        from src.scenario_config import ScenarioConfig, ScenarioValidator
    ...
    def test_is_valid_round_count_exceeds_max(self):
        """Test is_valid_round_count exceeds max."""
        from src.scenario_config import ScenarioConfig, ScenarioValidator
        ...
        is_valid, reason = validator.is_valid_round_count(75)
        assert is_valid is False
        assert "exceeds" in reason.lower()

    def test_validate_all_settings(self):
        """Test validate_all_settings."""
        from src.scenario_config import ScenarioConfig, ScenarioValidator

        player = Mock()
        player.game_config = Mock()
        player.game_config.current_scenario = "standard"
        player.game_config.test_scenario = ""
        player.game_config.starting_difficulty = 3
        player.game_config.difficulty_scaling = 0.5
        player.game_config.max_rounds_before_auto_victory = 50
...
class TestAIDecisionValidator:
    """Test AIDecisionValidator class."""

    def test_ai_decision_validator_init(self):
        """Test AIDecisionValidator initialization."""
        from src.npc_ai_config import NPCAIConfig, AIDecisionValidator
    ...
    def test_is_valid_flank_decision_flanking_disabled(self):
        """Test is_valid_flank_decision when flanking disabled."""
        from src.npc_ai_config import NPCAIConfig, AIDecisionValidator
        ...
        is_valid, reason = validator.is_valid_flank_decision(Mock(), Mock(), [])
        assert is_valid is False
        assert "disabled" in reason.lower()

    def test_is_valid_flank_decision_valid(self):
        """Test is_valid_flank_decision with valid conditions."""
        from src.npc_ai_config import NPCAIConfig, AIDecisionValidator
        ...
        target = Mock()
        ally = Mock()
        npc.combat_proximity = {target: 30.0}

        is_valid, reason = validator.is_valid_flank_decision(
            npc, target, [npc, ally]
        )
        assert is_valid is True
        assert "conditions met" in reason.lower()

    def test_is_valid_retreat_decision_valid(self):
        """Test is_valid_retreat_decision with valid conditions."""
        from src.npc_ai_config import NPCAIConfig, AIDecisionValidator
    ...
    def test_is_valid_flank_distance(self):
        """Test is_valid_flank_distance."""
        from src.npc_ai_config import NPCAIConfig, AIDecisionValidator
    ...
    def test_is_valid_retreat_priority(self):
        """Test is_valid_retreat_priority."""
        from src.npc_ai_config import NPCAIConfig, AIDecisionValidator
    ...
    def test_validate_all_settings_invalid_threshold(self):
        """Test validate_all_settings with invalid threshold."""
        from src.npc_ai_config import NPCAIConfig, AIDecisionValidator

        player = Mock()
        player.game_config = Mock()
        player.game_config.npc_flanking_threshold = 200.0
        player.game_config.npc_retreat_health_threshold = 0.3
        player.game_config.npc_flanking_distance_range = "20.0 to 40.0"
        ...
        assert len(issues) > 0
        assert "out of range" in issues[0].lower()
