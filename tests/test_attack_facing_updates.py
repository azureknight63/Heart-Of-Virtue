...
class TestAttackMovesFacing:
    ...
    def player(self):
        ...
        sword = Shortsword()
        p.eq_weapon = sword
        # Initialize combat_exp for weapon subtype
        if hasattr(sword, 'subtype') and sword.subtype not in p.combat_exp:
            p.combat_exp[sword.subtype] = 0
        p.combat_proximity = {}
        return p
    ...
    def test_shoot_bow_faces_target(self, player, enemy):
        ...
        # Equip a bow for this test
        from src.items import Shortbow  # type: ignore
        bow = Shortbow()
        player.eq_weapon = bow
