...
def test_missing_player():
    ...
    for x, y, desc in test_positions:
        ...
        if "Jean" in window.combatants_data:
            jean_data = window.combatants_data["Jean"]
            print(f"    [OK] Jean found in combatants_data")
            print(f"      Position: ({jean_data['position'].x}, {jean_data['position'].y})")
            print(f"      Health: {jean_data['health_percent']*100:.0f}%")
        else:
            print(f"    [ERROR] Jean NOT found in combatants_data!")
...
if __name__ == "__main__":
    print("=" * 60)
    print("Battlefield Player Debug Test")
    print("=" * 60)
