...
class DialogueConditionSerializer:
    ...
    def check_conditions(
        ...
        if condition.required_completed_dialogues:
            missing_dialogues = [
                d for d in condition.required_completed_dialogues
                if d not in player_completed_dialogues
            ]
            if missing_dialogues:
                return False, f"Must complete dialogue(s) first: {', '.join(missing_dialogues)}"
        
        # Check player level
        if condition.min_player_level and player_level < condition.min_player_level:
            return False, f"Player level too low (need {condition.min_player_level})"
...
class DialogueChoiceSerializer:
    ...
    def serialize(choice: DialogueChoice) -> Dict[str, Any]:
        ...
        return {
            ...
            "condition": DialogueConditionSerializer.serialize(choice.condition)
                if choice.condition else None,
...
class DialogueNodeSerializer:
    ...
    def serialize(node: DialogueNode) -> Dict[str, Any]:
        ...
        return {
            ...
            "condition": DialogueConditionSerializer.serialize(node.condition)
...
class ConversationHistorySerializer:
    ...
    def deserialize(data: Dict[str, Any]) -> ConversationHistory:
        ...
        Raises:
            KeyError: If required fields missing
        """
        required_fields = {"conversation_id", "npc_id", "player_id", "dialogue_id", "started_at"}
        if not required_fields.issubset(set(data.keys())):
            raise KeyError(f"Missing required fields: {required_fields - set(data.keys())}")
...
class DialogueContextSerializer:
    ...
    def deserialize(data: Dict[str, Any]) -> DialogueContext:
        ...
        """
        required_fields = {
            "conversation_id", "current_node", "available_choices", "conversation_history"
        }
        if not required_fields.issubset(set(data.keys())):
            raise KeyError(f"Missing required fields: {required_fields - set(data.keys())}")
        ...
        return DialogueContext(
            ...
            available_choices=[
                DialogueChoiceSerializer.deserialize(c) for c in data["available_choices"]
            ],
