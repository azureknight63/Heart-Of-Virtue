...
class AIDecisionValidator:
    ...
    def is_valid_flank_decision(self, npc, target, allies: list) -> Tuple[bool, str]:
        ...
        if distance < min_range or distance > max_range:
            return (
                False,
                f"Target outside flank range ({distance:.1f} not in {min_range}-{max_range})",
            )
    ...
    def is_valid_flank_distance(self, distance: float) -> Tuple[bool, str]:
        ...
        return (
            True,
            f"Distance in valid range ({distance:.1f} in {min_range}-{max_range})",
        )
    ...
    def validate_all_settings(self) -> Tuple[bool, list]:
        ...
        if threshold < 0 or threshold > 180:
            issues.append(
                f"Flanking threshold out of range: {threshold}° (must be 0-180°)"
            )
        ...
        if retreat_threshold < 0.0 or retreat_threshold > 1.0:
            issues.append(
                f"Retreat threshold out of range: {retreat_threshold} (must be 0.0-1.0)"
            )
