...
class MynxLLMMixin:
    ...
    _re_self_actions = re.compile(
        r"\b(batting|pawing|swatting|tapping) at (?:{name}|{pronoun})\b", re.IGNORECASE
    )
    ...
    def _get_llm_adapter(self):
        ...
        try:
            ...
            if avail is True:
                ...
                if _debug:
                    status = inst.debug_status() if hasattr(inst, "debug_status") else "ok"
    ...
    def _sanitize_mynx_llm_text(self, text: str, allowed_names) -> str:
        ...
        try:
            ...
            poss_adj = (pronouns.get("possessive_adjective")
                        or ("its" if pronoun == "it" else f"{pronoun}'s"))
            name = self.name
    ...
    def _enforce_pronouns_and_names(self, text: str, roster_set: set[str]) -> str:
        ...
        try:
            ...
            poss_mynx = (pronouns.get('possessive_adjective')
                         or pronouns.get('possessive') or 'its')
            ...
            jean_poss = (jean_pronouns.get('possessive_adjective')
                         or jean_pronouns.get('possessive') or 'his')

            def map_token(token, target):
                t = token.lower()
                if target == 'jean':
                    return jean_subj if t in ('he', 'she') else (
                        jean_obj if t in ('him', 'her') else (
                            jean_poss if t in ('his', 'hers') else jean_subj))
                if target == 'mynx':
                    return pron_mynx if t in ('he', 'she', 'him', 'her') else (
                        poss_mynx if t in ('his', 'hers') else pron_mynx)
                return 'they' if t in ('he', 'she') else (
                    'them' if t in ('him', 'her') else (
                        'their' if t in ('his', 'hers') else 'they'))
    ...
    def _gather_environment_lists(self):
        ...
        try:
            room_items = (getattr(room, 'items_here', None)
                          ...
                          or getattr(room, 'spawned', None) or [])
            ...
            room_objs = (getattr(room, 'objects_here', None)
                         or getattr(room, 'objects', None) or [])
            ...
            for npc_inst in getattr(room, 'npcs_here', []) or []:
                ...
                if isinstance(nm, str) and nm.strip() and nm != self.name:
                    ndesc = (getattr(npc_inst, 'description', None)
                             or getattr(npc_inst, 'discovery_message', None))
                    entry = self._normalize_ws(nm)[:60]
    ...
    def _build_pronoun_guidance(self, jean_pronoun_line: str, jean_snippet: str) -> str:
        ...
        mynx_possessive = pronouns.get('possessive_adjective') or pronouns.get('possessive') or 'its'
        try:
            pg = []
    ...
    def _build_llm_context(
        self, roster_set: set[str], prompt: str, jean_pronoun_line: str, jean_snippet: str
    ) -> str:
        """Assemble the full context string to send to the LLM."""
        room_desc_raw = (
            getattr(self.current_room, 'description', '')
            if getattr(self, 'current_room', None) else ''
        )
    ...
    def _check_and_correct_mynx_text(self, text: str, prompt: str, roster) -> str | None:
        ...
        try:
            ...
            pronouns = self.pronouns if hasattr(self, 'pronouns') else {}
            pronoun = pronouns.get('personal', 'it')
            poss_adj = pronouns.get('possessive_adjective') or pronouns.get('possessive') or 'its'
    ...
    def interact_with_player(self, player, prompt: str | None = None, structured: bool = False):
        ...
        if p in ("pet", "stroke", "scritch"):
            ...
            action_print = (
                ...
                if item_name else "Jean tries to play with the mynx."
            )
        elif p:
            ...
            try:
                if structured:
                    ...
                    if isinstance(result, dict) and isinstance(result.get('description'), str):
                        if _debug:
                            try:
                                print(f"[MYNX_LLM_DEBUG] Raw structured description: {result.get('description')}")
                            except Exception:
                                pass
                        desc = self._sanitize_mynx_llm_text(result['description'], roster_set)
                    ...
                    if isinstance(text_resp, str) and text_resp:
                        ...
                        if checked is not None:
                            self._llm_last_response = {
                                'action': 'narrate', 'intensity': 'low',
                                'description': checked, 'duration_seconds': 2,
        ...
        if p in ("pet", "stroke", "scritch"):
            ...
            structured_obj = {"action": "groom", "intensity": "gentle",
                              "description": text, "duration_seconds": 2, "audible": "soft purr/chitter"}
        elif p in ("feed", "offer food", "give food"):
            variations = [
            ...
            text = random.choice(variations)
            structured_obj = {"action": "take_food", "intensity": "medium",
                              "description": text, "duration_seconds": 3, "audible": "happy chitter"}
            ...
            text = random.choice(variations)
            structured_obj = {"action": "play", "intensity": "high",
                              "description": text, "duration_seconds": 4, "audible": "rapid chitters"}
        else:
            variations = [
            ...
            text = random.choice(variations)
            structured_obj = {"action": "investigate", "intensity": "low",
                              "description": text, "duration_seconds": 3, "audible": "soft chitter"}
