...
class Teleport(Action):
    def __init__(self):
        super().__init__(method=Player.teleport, name="Teleport", hotkey=('tele', 'teleport'), color='silver')


class Alter(Action):  # change a switch/variable
    def __init__(self):
        super().__init__(method=Player.alter, name="Alter", hotkey=('alt', 'alter'), color='silver')


class Showvar(Action):  # list all switches/vars
    def __init__(self):
        super().__init__(method=Player.vars, name="Showvar", hotkey=('sv', 'showvar'), color='silver')


class Supersaiyan(Action):
    def __init__(self):
        super().__init__(method=Player.supersaiyan, name="Supersaiyan", hotkey=('ss', 'supersaiyan'), color='silver')


class TestEvent(Action):
    def __init__(self):
        super().__init__(method=Player.testevent, name="TestEvent", hotkey=('te', 'test', 'testevent'), color='silver')


class SpawnObj(Action):
    def __init__(self):
        super().__init__(method=Player.spawnobject, name="SpawnObj", hotkey=('so', 'spawn', 'spawnobject'), color='silver')
...
class RefreshMerchants(Action):  # debug utility to refresh all merchant inventories
    def __init__(self):
        super().__init__(method=Player.refresh_merchants, name="Refresh Merchants", hotkey=(
            'rm', 'refreshmerchants', 'merchrefresh', 'updatemerchants'), color='silver')