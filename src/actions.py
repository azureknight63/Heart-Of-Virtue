...
class Teleport(Action):
    def __init__(self):
        super().__init__(method=Player.teleport, name="Teleport", hotkey=('tele', 'teleport'), debug=True, color='silver')


class Alter(Action):  # change a switch/variable
    def __init__(self):
        super().__init__(method=Player.alter, name="Alter", hotkey=('alt', 'alter'), debug=True, color='silver')
...
class Supersaiyan(Action):
    def __init__(self):
        super().__init__(method=Player.supersaiyan, name="Supersaiyan", hotkey=('ss', 'supersaiyan'), debug=True, color='silver')


class TestEvent(Action):
    def __init__(self):
        super().__init__(method=Player.testevent, name="TestEvent", hotkey=('te', 'test', 'testevent'), debug=True, color='silver')


class SpawnObj(Action):
    def __init__(self):
        super().__init__(method=Player.spawnobject, name="SpawnObj", hotkey=('so', 'spawn', 'spawnobject'), debug=True, color='silver')
...
class RefreshMerchants(Action):  # debug utility to refresh all merchant inventories
    def __init__(self):
        super().__init__(method=Player.refresh_merchants, name="Refresh Merchants", hotkey=(
            'rm', 'refreshmerchants', 'merchrefresh', 'updatemerchants'), debug=True, color='silver')