...
class GridValidator:
    ...
    def validate_position(
        self, x: int, y: int, strict: bool = False
    ) -> Tuple[bool, str]:
    ...
    def validate_distance(
        self, distance: float, min_allowed: float = 0.0
    ) -> Tuple[bool, str]:
        ...
        max_distance = self.coord_config.get_distance_between_points(
            0, 0, width, height
        )
    ...
    def validate_zone_bounds(
        self, zone_min: Tuple[int, int], zone_max: Tuple[int, int]
    ) -> Tuple[bool, str]:
