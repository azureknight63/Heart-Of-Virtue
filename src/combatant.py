...
class Combatant:
    ...
    #: via ``terrain.attach``; None outside a terrain fight. Declared here so
    #: Player and NPC advertise the same slot.
    combat_terrain = None
    #: Cell this combatant last stood on, for hazard entry detection
    #: (``terrain.apply_entry_effects``); reset by ``terrain.attach``.
    _terrain_last_cell = None
    #: Per-selection cache for ``NPCAIConfig._terrain_context``.
    _terrain_ai_context = None
    ...
    def __getstate__(self):
        ...
        # Terrain is per-fight and regenerated by the adapter on every combat
        # start; combat never resumes across a load, so the grid (and the
        # per-unit hazard/AI scratch keyed to it) is dropped from saves.
        for key in ("combat_terrain", "_terrain_last_cell", "_terrain_ai_context"):
            state.pop(key, None)
        return state
