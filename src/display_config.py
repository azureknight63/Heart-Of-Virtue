...
def display_enemy_status(enemy, display_config):
    ...
    if (
        display_config.should_show_positions()
        and hasattr(enemy, "combat_position")
        and enemy.combat_position
    ):
    ...
    if display_config.should_show_distance() and hasattr(
        display_config.player, "combat_proximity"
    ):
...
def display_damage_modifier_info(attacker, defender, damage_base, display_config=None):
    ...
    if hasattr(attacker, "combat_position") and hasattr(defender, "combat_position"):
        if attacker.combat_position and defender.combat_position:
            ...
            modifiers.append(
                "Flanked (1.5x)" if False else "Standard (1.0x)"
            )  # Placeholder
...
def display_accuracy_modifier_info(
    attacker, defender, base_accuracy, display_config=None
):
