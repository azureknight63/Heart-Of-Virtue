...
def combat(player, event_config: Optional[CombatEventConfig] = None):
        ...
        for each_ally in player.combat_list_allies:
            ...
            for each_enemy in player.combat_list:
                ...
                for (
                    ...
                    each_enemy.combat_proximity
                ):  # Remove any dead allies from combat proximity;
    ...
    battlefield_window.update_all_combatants(
        player, player.combat_list_allies, player.combat_list
    )
    battlefield_window.update_display()

    while (
        True
    ):  # combat will loop until there are no aggro enemies or the player is dead
            ...
            for i, move in enumerate(viable_moves):
                if i == selected_move:
                    if player.fatigue >= move.fatigue_cost and move.current_stage == 0:
                        ...
                        cprint(
                            "Jean will need to rest a bit before he can do that.",
                            "red",
                        )
