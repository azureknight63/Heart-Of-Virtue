...
class MemoryFlash(Event):
    ...
    def __init__(
        ...
        super().__init__(
            name=name, player=player, tile=tile, repeat=repeat, params=None
        )
    ...
    def process(self, user_input=None):
        ...
        if not self.repeat:
            if (
                hasattr(self, "tile")
                and self.tile
                and self in getattr(self.tile, "events_here", [])
            ):
                self.tile.events_here.remove(self)
            elif (
                hasattr(self, "player")
                and self.player
                and self in getattr(self.player, "combat_events", [])
            ):
...
class Effect(Event):
    def __init__(self, name, player):
        super().__init__(
            name=name, player=player, tile=player.tile, repeat=False, params=None
        )
...
class GoldFromHeaven(
    Event
):  # Gives the player a certain amount of gold... for testing? Or just fun.
    def __init__(
        self, player, tile, params=None, repeat=False, name="Gold From Heaven"
    ):
        super().__init__(
...
class StMichael(Shrine):
    def __init__(
        ...
        params=None,
        repeat=False,
        name="Shrine of St Michael the Archangel",
    ):
        super().__init__(
            name=name, player=player, tile=tile, repeat=repeat, params=params
        )
    ...
    def process(self, user_input=None):
        ...
        drop = self.tile.spawn_item(
            self.available_choices[selection][1], amt=1, hidden=False, hfactor=0
        )
        functions.add_random_enchantments(drop, 1)
        cprint(
            "There's a brief flash of light (or was it imagined?) \nSuddenly, at the foot of the shrine, "
            "there sits a {}.".format(drop.name),
            "cyan",
        )
...
class NPCSpawnerEvent(Event):
    ...
    def __init__(
        ...
        params=None,
        repeat: bool = False,
        npc_cls: type[NPC] = None,
        count: int | None = None,
        name: str = "NPCSpawnerEvent",
    ):
        super().__init__(
            name=name, player=player, tile=tile, repeat=repeat, params=params
        )
        ...
        if params:
            try:
                ...
                if (
                    len(params) > 2
                    and isinstance(params[2], (list, tuple))
                    and len(params[2]) == 2
                ):
...
class WhisperingStatue(Event):
    def __init__(
        self, player, tile, params=None, repeat=False, name="The Whispering Statue"
    ):
        super().__init__(
            name=name, player=player, tile=tile, repeat=repeat, params=params
        )
    ...
    def process(self, user_input=None):
        ...
        if choice == "1":
            ...
            # Reward
            cprint(
                f"{self.player.name} found a pouch of Gold!", "green", attrs=["bold"]
            )
            ...
            time.sleep(1)
            cprint(
                '"Foolishness invites destruction," the voice hisses.',
                "red",
                attrs=["bold"],
            )
