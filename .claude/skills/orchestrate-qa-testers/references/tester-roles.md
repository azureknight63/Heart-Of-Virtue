...
- **Where scenes arrive.** A no-input scene comes back in the `events_triggered` of the move or interact that caused it. Only needs-input events wait in `/api/world/events/pending`. Two runs filed a Critical "scene never fires" off an empty pending list: the 2026-09-24 Gorran farewell, and 2026-09-25's Devet / Iron & Oath / Mara observation. All of them had fired.
- **How loot works.** `end_state.items_dropped` lists the drops, and `POST /api/combat/collect-loot` takes only the `item_names` you pass. An empty list collects nothing. On 2026-09-25, "mobs never drop anything" came from 21 of 22 fights that did drop, collected with empty lists.
- **What the status echo is.** After a won fight, `GET /api/combat/status` echoes held story scenes tagged `post_combat: true`; the SPA filters them, and collect-loot delivers them. Describe a fix's contract from its tests, not from memory: a primer that said "must not appear in status" produced a false #683 Critical.
- **What a test session can't show.** Test sessions get the generic save 403 before any other save check (e.g. #690's death refusal), so don't ask testers to verify a save-path fix; its unit tests cover it.
