...
Your `context_paths` cover the **enclosing top-level symbol** (the class or function containing the change, plus its imports). If you need additional context from the same file — for example, to check a naming convention used elsewhere in the file — use `Read` to fetch the relevant lines.
