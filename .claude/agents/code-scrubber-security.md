...
- **This codebase's save/load posture — read this before grading any deserialization path.** `src/secure_pickle.py` implements real hardening (two strict gates in `find_class`, an auto-derived engine allow-list, a curated legacy set, an integrity header, a size cap, a sandboxed subprocess loader). Strict enforcement is **the intended production posture**, but it is gated on `HOV_STRICT_UNPICKLE`, which is currently unset in every config — so on a default in-process load the allow-list does not gate at all, and what protects the process is the sha256 header, the 5 MB cap and legacy-module rewriting. Review every new or changed deserialization path **as though strict mode is on**, and **file a finding for any path that only holds with strict mode off**. "SafeUnpickler handles it" is not a reason to pass a path; it is a claim to verify against `src/secure_pickle.py` and `.claude/rules/saves-persistence.md`.
