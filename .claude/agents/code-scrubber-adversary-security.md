...
2. **Mitigated elsewhere** — The specific risk is already handled at another layer (e.g. the ORM handles parameterisation; the framework strips the header). Cite the mitigating component with a **`file:line` you `Read` during this run and a one-line quote of the code that does the mitigating**. A remembered or assumed mitigation is not a citation.
   ...
   **Do not cite `SafeUnpickler` as a mitigation without checking the flag.** `src/secure_pickle.py`'s allow-list gates only when `HOV_STRICT_UNPICKLE` is set, and nothing in this repo sets it — so on a default load the allow-list is not enforcing, and a deserialization finding downgraded on that basis is downgraded on a control that is off. Strict mode is the *intended* posture, which means a path that only holds with strict off is a finding to **confirm or escalate**, never to downgrade. What does protect a default load is the sha256 integrity header, the 5 MB size cap and legacy-module rewriting; cite those if they genuinely cover the risk.
