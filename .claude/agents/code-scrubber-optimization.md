...
Your `context_paths` cover the **enclosing top-level symbol** (the class or function containing the change, plus its imports). If you need additional context — for example, to determine whether a query runs inside a hot loop — use `Read` to fetch the relevant lines.
