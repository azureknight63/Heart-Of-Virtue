...
- **`say(..., thought=True)` beats are first-person interior monologue, not third-person narration.** The client (`ConversationStage.jsx`) renders a `thought` beat italicized with no reaction from other portraits specifically because it's Jean's internal voice — "I notice which way it's moving before I step inside," never "He notes which way it's moving before he steps inside." Issue #593: nearly every authored thought beat in `ch01.py`/`ch02.py` was written in third-person narrator voice despite the mechanism distinguishing it as internal; they were rewritten to first person without changing any narrative fact. A thought beat with no personal pronoun at all (a sensory fragment like `"Pain — sudden, immediate, real."`) doesn't need one manufactured — only third-person markers (`he`/`his`/`Jean was...`) need converting.
