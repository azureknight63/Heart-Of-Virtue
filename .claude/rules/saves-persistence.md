...
- **Strict mode is the default; the env var only turns it off.** `safe_pickle_load(fp)` is called with `strict=None`, which resolves through `strict_mode_enabled()` → the `HOV_STRICT_UNPICKLE` env var, and that function now returns True unless the value is an explicit opt-out (`0`/`false`/`no`/`off`, per `STRICT_OPT_OUT_VALUES`). So on a default load `SafeUnpickler.find_class` admits only globals from engine modules plus the curated `_SAFE_STDLIB` set, rejects everything else with `RestrictedUnpicklingError`, and synthesizes a placeholder only for the curated `LEGACY_ALLOWED_MISSING`. `find_class`'s `getattr(self, "strict", True)` fallback means even an instance built through `__new__` fails closed.

  It was the other way round until the beta flip (2026-09-12), and the consequences of that history still matter when reading older code and tests: anything that round-tripped a *test-local* class through the real save path only ever worked because the gate was inert, and now fails. Two such tests existed and are fixed — `test_game_service_expanded.py` now round-trips real `src.states.State` instances instead of module-local stand-ins. Placeholder synthesis is the opt-out path: tests that need it either pass `strict=False` or set the env var, and they say which.

  Retiring an engine class is now a save-breaking change unless you add it to `LEGACY_ALLOWED_MISSING`. That trade is accepted in beta — save compatibility is explicitly not a constraint at this stage — but it is a decision to make deliberately rather than discover from a player's load failure.

- **Checklist for removing or renaming a persisted class.** `LEGACY_ALLOWED_MISSING` is empty, so there is currently no class the loader will forgive. Before deleting or renaming anything that can appear in a save (items, NPCs, states, events, tiles, moves):
  1. Decide whether existing saves should survive it. In beta the answer may legitimately be no — say so in the commit rather than leaving it implied.
  2. If they should, add `("src.<module>", "<ClassName>")` to `LEGACY_ALLOWED_MISSING` **in the same commit as the removal**. Added later, it does not help the saves that already failed.
  3. A rename is a removal plus an addition: the old `(module, name)` needs the entry, not the new one.
  4. Regenerate the manifest (`python tools/gen_allowlist_manifest.py`) and run `python tools/save_fuzzer.py` — the fuzzer exercises real engine classes, so a removal that breaks reconstruction shows up there rather than in a player's load.
  5. A curated placeholder is a tombstone, not a fix: the object loads inert (`_legacy_placeholder=True`). If game logic must still *work* with that data, you need a migration, not an allow-list entry.
