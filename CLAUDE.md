...
- Every fight lays a per-cell `TerrainGrid` (kinds `open/rough/hazard/shelf/boulder/wall/cliff` + elevation) under the `src/positions.py` coordinate grid, generated per combat from the player's **map region** (`terrain.region_for_player`: `metadata.terrain_region` on the map JSON, else a map-name hint; unknown maps and every `*test*` map get the flat `arena`, so combat-test scenarios are unchanged). Generators are seeded, connectivity-enforced (largest passable component wins; pockets are filled), and keep the scenario's spawn zones clear.
- **The grid lives on every combatant as `combat_terrain`** (one shared object per fight, attached by `ApiCombatAdapter.initialize_combat` and re-attached to reinforcements), never on a module global. `terrain.grid_for(unit)` is the only accessor: it returns None for a trivial grid, a MagicMock, or anything that isn't a real `TerrainGrid`, and every consumer degrades to the pre-terrain behaviour on None. A reinit keeps the fight's grid (and its dimensions win over a recomputed `combat_grid_size`).
- Mechanics flow through the existing funnels, not new call sites: `_apply_to_hit_modifiers` adds `terrain.apply_accuracy` (flat cover points past `COVER_MIN_DISTANCE_FT` = melee reach, ±10 elevation), `apply_facing_damage` multiplies by `terrain.damage_multiplier` (elevation), the `positions` movers take a `terrain=` kwarg (A* routing, per-cell cost, retreat scoring), `NPCAIConfig.get_terrain_move_bonus` / `get_flank_position_angle` read the same grid, and `_synchronize_distances` fires hazard entry effects once per cell change. `terrain.engagement(attacker, defender)` is the single source for the numbers shown on target cards; the client never re-derives cover or elevation.
- Wire: `battle_state["terrain"]` (compact rows of kind codes + elevation digits, region palette, legend) ridden to `BattlefieldGrid` as an explicit `terrain` prop from `Battlefield.jsx` (same trap as `map_size`: the grid gets a beat state); per-combatant `terrain` (standing-on) and `sprite_key`; per-target `terrain` (engagement). All pinned in `tests/test_wire_field_contract.py`.
- Art: sprites and region tilesets come from an external image model via the prompt pack in `docs/development/art-prompts/` (generated by `tools/art_prompts.py`) and `tools/sprite_intake.py`, which writes `frontend/public/assets/sprites/manifest.json`. A combatant/region with no manifest entry keeps the glyph token / procedural terrain fills. See `docs/development/art-pipeline.md`; `tests/test_art_pipeline.py` fails if `REGION_PALETTES` and the tileset prompts disagree.
...
- Battlefield terrain (all three phases): `src/terrain.py` per-cell terrain layer with region generators (Verdette Caverns, Eastern Descent, Dark Grotto, Mineral Pools, Grondia, Wailing Badlands), obstacle pathing, ranged cover, elevation, hazard entry effects, terrain-aware NPC AI; surfaced on the wire and drawn by `TerrainLayer`/`TerrainLegend` with cover/elevation labels on target cards; animated sprite tokens (`SpriteToken`, `useSpriteManifest`) and tileset-backed terrain fed by the Gemini prompt pack + `tools/sprite_intake.py` (placeholder strips committed for jean/slime/gorran). See "Battlefield terrain" above and `docs/development/art-pipeline.md`.
