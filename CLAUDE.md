...
**Review-gate remediation is the one carve-out**: fixes applied by `/code-review` or `/code-scrubber` to land a finding don't need a preceding failing test, but they must leave the suite green, and if the fix changes behaviour it still needs a regression test before the gate passes. **The carve-out does not extend to Security fixes** — those need the reproduction executed before and after, because a security fix whose effect was never observed is the case the research says is most often wrong (26% of LLM security patches were clean; 37.5% of the apparently-successful ones left the root cause reachable).
...
**For a *fix*, pick the rung that can observe the regression the fix might cause, not just the one that observes the fix.** These are usually different rungs, and the second is the one that gets skipped. A patch that closes a bug and quietly moves behaviour passes every test aimed at the bug — measured at 20.1% of LLM-generated security patches in the research behind `docs/development/flawed-patch-review-hardening-plan.md`. This applies to fixes a review skill applies to itself, which is exactly where nobody else is checking.
...
- `/api/debug/*` (`routes/debug.py`) registers only under `app.config["TESTING"]`; never add a debug route outside that gate. Any new deserialization path goes through `src/secure_pickle.py`'s `SafeUnpickler`, whose allow-list **enforces by default** — `HOV_STRICT_UNPICKLE` is an opt-out (`0`/`false`/`no`/`off`) and nothing else, including a typo, disables it (`.claude/rules/saves-persistence.md`). A path that only works with strict off, or a `strict` default weakened to False, is a defect. Retiring an engine class now breaks saves naming it unless it is curated into `LEGACY_ALLOWED_MISSING`.
...
- **Save compatibility is suspended for beta** (maintainer call, 2026-09-12): breaking old saves is an acceptable cost while the game is pre-release, which is what allowed strict unpickling to become the default. Keep the *hygiene* — persisted classes still want backward-compatible defaults, format changes still regenerate the allow-list manifest and still get a fixture/fuzz run (`.claude/rules/saves-persistence.md`) — but a change is no longer blocked solely because it invalidates existing saves. Revisit this before first release.
