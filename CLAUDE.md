...
- **Remaining:** `moves/*` still has `input()` in terminal move `execute()`/`prep()` paths (target/distance/arrow selection). These are **combat-adapter territory** — the adapter drives moves via `cast()`/`advance()` plus its own structured `select_target`/`select_number`/`select_direction` commands, not the event net, so they're a separate teardown. A few non-event terminal command methods (`Player.menu`, `Player.commands`, terminal action classes in `actions.py`/`tiles.py`'s `callerIsApi=False` list) also linger but are not API-invoked.
