...
- `/api/debug/*` (`routes/debug.py`) registers only under `app.config["TESTING"]`; never add a debug route outside that gate. Any new deserialization path goes through `src/secure_pickle.py`'s `SafeUnpickler` — whose allow-list only *enforces* under `HOV_STRICT_UNPICKLE`, which nothing sets (`.claude/rules/saves-persistence.md`). **Strict is the intended production posture, and the default-off state is a migration artifact, not a decision**: review and design new deserialization paths as though strict mode is on, and treat a path that only holds with it off as a defect. "SafeUnpickler handles it" is not a reason to pass a path — the allow-list isn't gating on a default load.
