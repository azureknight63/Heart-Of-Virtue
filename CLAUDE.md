...
### Battlefield terrain (`src/terrain.py`)
- Every fight lays a per-cell `TerrainGrid` (kinds `open/rough/hazard/shelf/boulder/wall/cliff` + elevation) under the `src/positions.py` coordinate grid, generated per combat from the player's **map region** (`terrain.region_for_player`: `metadata.terrain_region` on the map JSON, else a map-name hint; unknown maps and every `*test*` map get the flat `arena`, so combat-test scenarios are unchanged). Generators draw only from `random.Random(seed)`, dimensions are clamped to `MAX_GRID_DIM`, pockets outside the largest passable component are filled, and each spawn zone is guaranteed a passable core joined to it (not a fully clear zone).
- **The grid lives on every combatant as `combat_terrain`** (a `Combatant` class slot, attached by `ApiCombatAdapter.initialize_combat` via `_spawn_on_terrain` and re-attached to reinforcements; stripped from pickles by `Combatant.__getstate__` and cleared by `load_save`/`_teardown_combat_roster`). `terrain.grid_for(unit)` is the *mechanics* accessor and returns None for a trivial grid, a MagicMock, or anything that isn't a real `TerrainGrid`, so every consumer degrades to the pre-terrain behaviour; the adapter's `combat_terrain` property is the one place that reads the raw grid, trivial or not, because the wire payload and `map_size` need its dimensions. A reinit keeps the fight's grid and its dimensions; `TerrainGrid.__setstate__` re-validates anything restored from a save.
- Mechanics flow through the existing funnels, not new call sites: `_apply_to_hit_modifiers(attacker, defender, hit, move=None)` adds `terrain.apply_accuracy` (flat cover points for **ranged** moves only — `Move.is_ranged`, inferred from `web_animation == "projectile"` unless a subclass sets `ranged`; a wall on a ranged line returns `NO_LINE_OF_SIGHT`, an auto-miss that also makes the target unselectable on the card; ±10 elevation for everyone), `apply_facing_damage` multiplies by `terrain.damage_multiplier` (elevation), the `positions` movers take a `terrain=` kwarg (A* routing, per-cell cost, retreat scoring with the same first-step floor advances get), `NPCAIConfig.get_terrain_move_bonus` / `get_flank_position_angle` read the same grid, every combat NPC carries `TakeGround` (walks to `terrain.best_ground`: cover from the target, higher ground, off a hazard), and `_apply_terrain_hazards` (from `_synchronize_distances`) rolls hazard entry effects once per hazard *entry*, sampled on the end-of-beat cell (crossing slime mid-path never rolls). `terrain.engagement(attacker, defender, ranged=)` is the single source for the numbers shown on target cards; the client never re-derives cover or elevation. When a callsite has no move in hand the attacker's `current_move` stands in; with neither, cover falls back to the `COVER_MIN_DISTANCE_FT` distance proxy.
- Wire: `battle_state["terrain"]` (compact rows of kind codes + elevation digits, region palette/label, legend incl. an `effect` flag for hazards) ridden to `BattlefieldGrid` as an explicit `terrain` prop from `Battlefield.jsx` through `useStableTerrain` (one object per `combat_id`, because the poll re-sends it and the grid's memos key on identity); per-combatant `terrain` (standing-on) and `sprite_key`; per-target `terrain` (engagement). All pinned in `tests/test_wire_field_contract.py`. `TerrainLayer`/`TerrainLegend` live inside `BattlefieldGrid.jsx`.
- Art: sprites and region tilesets come from an external image model via the prompt pack in `docs/development/art-prompts/` (generated by `tools/art_prompts.py`, which owns the spec, the roster slugs = `CombatantSerializer.sprite_key`, and the file-name parsers) and `tools/sprite_intake.py`, which validates delivered file names against the roster, keys the magenta by hue (JPEG drift and dark gridlines included), locates cells from the content rather than an even split (`--grid even` is the escape hatch), resamples a wrong frame count to the spec's, normalises each sheet with one crop and one scale, and writes `frontend/public/assets/sprites/manifest.json` (the client re-validates every path before it reaches a CSS `url()`). A combatant/region with no manifest entry keeps the glyph token / procedural terrain fills. See `docs/development/art-pipeline.md`; `tests/test_art_pipeline.py` fails if `REGION_PALETTES` and the tileset prompts disagree in either direction.
...
- Battlefield terrain (all three phases): `src/terrain.py` per-cell terrain layer with region generators (Verdette Caverns, Eastern Descent, Dark Grotto, Mineral Pools, Grondia, Wailing Badlands), obstacle pathing, ranged-only cover with walls blocking line of sight outright, elevation, hazard entry effects, terrain-aware NPC AI incl. the `TakeGround` seek-cover move; surfaced on the wire and drawn by `TerrainLayer`/`TerrainLegend` (inside `BattlefieldGrid.jsx`) with cover/elevation labels on target cards; animated sprite tokens (`SpriteToken`, `useSpriteManifest`) and tileset-backed terrain fed by the Gemini prompt pack + `tools/sprite_intake.py` (placeholder strips committed for jean/slime/gorran). See "Battlefield terrain" above and `docs/development/art-pipeline.md`.
