...
- **To-hit arithmetic lives in `src/moves/_base.py`, never in the API layer.** `to_hit_chance(user, target, base=, floor=)` is the real roll; `attacker_accuracy(finesse, intelligence, base=)` is the display-only rating the character sheet renders. The expression was previously inlined at ~20 call sites *and* re-implemented in `game_service`, which is the exact reimplementation CLAUDE.md forbids. Two traps if you touch this: (1) the sites are **not** uniform — `base` is 98 for most moves, 85 for the polearm/scythe sweeps and the sword's spin attacks (`WhirlAttack`/`VertigoSpin` — *not* `Riposte`, which takes the default 98; an earlier version of this note said otherwise and reconciling code to it would silently drop Riposte's accuracy by 13 points), and 90/95/105 for particular dagger and NPC moves, while `floor` is 5, 1, or absent; (2) **term order is load-bearing** — `base - target.finesse` is evaluated before the weighted attacker terms are added, and folding the attacker terms first shifts the truncated result by one point for ~0.7% of integer stat pairs, so `to_hit_chance` must never be "simplified" into `attacker_accuracy(...) - target.finesse`. Situational modifiers (ranged accuracy decay, Hawkeye, Aimed Shot's flat bonus, the crossbow close-range halving) stay at the call sites because several interpose before `_apply_to_hit_modifiers` and the clamps.
