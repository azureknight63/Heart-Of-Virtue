...
- `hov_local_autosave` hardened and de-fanged: `frontend/src/utils/localSave.js` validates the blob as untrusted input (size cap applied before `JSON.parse`, own-key checks so a polluted prototype cannot satisfy a missing field, `__proto__`/`constructor` rejection, per-field bounds, control-character stripping) and it is cleared on logout and on a 401. It is still **write-only** — nothing restores from it — so it no longer appears as a selectable save row. See issues #487 (the false promise) and #489 (retire it by closing the cloud-autosave gap server-side).
