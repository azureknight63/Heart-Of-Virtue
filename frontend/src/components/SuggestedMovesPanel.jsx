...
export default function SuggestedMovesPanel({ suggestions = [], suggestionsLoading = false, lastOutcome = "", lastMoveViable = false, onSuggestClick, isPlayerTurn = false, onTargetHover, isMobile = false, onPause, onRequestSuggestions, blockedReasonFor }) {
    ...
    // Mobile + collapsed renders as a compact tap-to-expand strip. Both
    // presentations share ONE tree — wrapper, header button, body region — and
    // differ only in styles and the header's label, so toggling never remounts
    // the focused <button> and keyboard focus stays on it (issue #674). The
    // body region stays mounted (empty while collapsed) so aria-controls always
    // resolves (issue #640).
